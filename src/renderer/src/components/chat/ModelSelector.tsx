...
const ModelPickerModal: React.FC<ModalProps> = ({
  ...
  const [leftProvider, setLeftProvider] = useState<string>(currentProvider);
  ...
  const rightModels = useMemo(
    () => models.filter((m) => m.providerId === leftProvider),
    [models, leftProvider]
  );
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0' }}>
              {visibleProviders.map((p) => {
                ...
                const noKeyNeeded = !p.apiKeyName;
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ flexShrink: 0, lineHeight: 1, display: 'flex', alignItems: 'center' }}>
                      {noKeyNeeded ? (
                        <span style={{ fontSize: 9, color: 'var(--text3)' }}>本地</span>
                      ) : hasKey ? (
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0', minWidth: 0 }}>
            {rightModels.length === 0 ? (
              <div style={{
                ...
                此提供商暂无模型<br />
                <span style={{ fontSize: 11 }}>请前往设置中添加</span>
              </div>
            ) : (
              rightModels.map((opt) => {
...
export const ModelSelector: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (const p of providers) {
      ...
      getApiKey(p.id).then((key) => {
        setKeyStatus((prev) => ({ ...prev, [p.id]: !!key }));
      });
    }
  }, [providers, getApiKey]);

  const currentModel: ProviderModel | undefined = models.find(
  ...
  const keyMissing = !currentKeyLoading && !currentKeyOk;
  ...
  return (
    <>
      <button
        ...
        title={keyMissing ? `${currentLabel}: ${t('model_selector.no_key_warning')}` : t('model_selector.select_model')}
        style={{
          display: 'flex', alignItems: 'center', gap: 4,
          padding: '4px 8px', fontSize: 12,
          color: 'var(--text2)',
          backgroundColor: open ? 'var(--surface2)' : 'transparent',
          border: `1px solid ${keyMissing ? '#fca5a5' : 'var(--border)'}`,
          borderRadius: 'var(--r)', cursor: 'pointer',
          whiteSpace: 'nowrap', fontFamily: 'var(--sans)',
          transition: 'background-color 0.1s, border-color 0.1s',
        }}
      >
        {keyMissing && <AlertTriangle size={11} style={{ color: '#ef4444', flexShrink: 0 }} />}
        <span style={{ color: keyMissing ? '#ef4444' : 'var(--text)' }}>
          {currentLabel}
        </span>
        <ChevronDown size={12} />
      </button>
...
export interface ModelOption { provider: string; model: string; label: string; tag: string }
